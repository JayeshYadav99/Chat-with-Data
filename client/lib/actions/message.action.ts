...
export async function clearMessages(chatId: string) {
  try {
    ...
    if (result.deletedCount > 0) {
      return {
        success: true,
        message: `Deleted ${result.deletedCount} messages from chat ${chatId}`,
      };
    } else {
      return {
        success: false,
...
export async function replicateMessages(
  ...
  try {
    ...
    const newMessages = await Promise.all(
      originalMessages.map(async (msg: { content: string; role: string }) => {
    ...
    return {
      success: false,
      message: "An error occurred while replicating messages",
    };
  }
}
