...
const ChatUI = ({chats,currentChat}:any) => {
  ...
  return (
    ...
    <ChatSidebar chats={chats}  chatUrl={currentChat?.pdfUrl} onSelectPdf={handleSelectPdf}  isPdfVisible={isPdfVisible} />
    ...
    <motion.div
      ...
      <div className={`${isPdfVisible ? "w-1/2" : "w-full"}  p-2 h-screen overflow-hidden`}>
        ...
        <ChatPanel    chatSource={currentChat?.source}   currentChat={currentChat} isShare={false} />
      </div>
    </main>
  </div></div>
  )
